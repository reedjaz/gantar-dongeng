    ...
    const promises = assets.map(src => {
        if (src.endsWith('.mp3')) {
            return new Promise(resolve => {
                ...
                if (window.OFFLINE_CONTENT && window.OFFLINE_CONTENT[normalizedSrc]) {
                    audio.src = window.OFFLINE_CONTENT[normalizedSrc];
                } else {
                    audio.src = src;
                }
            });
        } else if (src.endsWith('.json')) {
            return fetchOrBundle(src)
                .then(res => res.json())
                .then(() => updateProgress())
...
fetchOrBundle('scene/splash.html')
    .then(response => {
